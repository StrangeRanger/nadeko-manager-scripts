...
echo "8. Exit"
echo -rn "Choose [1] to Download, [2 or 3] to Run, [6 and 7] for pm2 setup/startup (see README) or [8] to Exit."
while [ $choice -eq 9 ]; do
    read choice
    if [ $choice -eq 1 ]; then
        echo ""
        echo "Downloading NadekoBot, please wait."
        wget -N https://github.com/"$installer_repo"/"$installer_branch"/nadeko_installer_latest.sh && bash "$root/nadeko_installer_latest.sh"
        echo ""
        bash "$root/linuxAIO.sh"
    elif [ $choice -eq 2 ]; then
        echo ""
        echo "Running Nadeko Normally, if you are running this to check Nadeko, use .die command on discord to stop Nadeko."
        wget -N https://github.com/"$installer_repo"/"$installer_branch"/nadeko_run.sh && bash "$root/nadeko_run.sh"
        echo ""
        ...
        sleep 5s
        wget -N https://github.com/"$installer_repo"/"$installer_branch"/NadekoAutoRestartAndUpdate.sh && bash "$root/NadekoAutoRestartAndUpdate.sh"
        echo ""
        ...
        wget -N https://github.com/"$installer_repo"/"$installer_branch"/nadekoautoinstaller.sh && bash "$root/nadekoautoinstaller.sh"
        echo ""
        echo "Welcome back..."
        sleep 2s
        bash "$root/linuxAIO.sh"
...
\"TotalShards\": 1
        ...
        wget -N https://github.com/"$installer_repo"/"$installer_branch"/nadekopm2setup.sh && bash "$root/nadekopm2setup.sh"
        echo ""
        echo "Welcome back..."
        sleep 2s
        bash "$root/linuxAIO.sh"
        ...
        wget -N https://github.com/"$installer_repo"/"$installer_branch"/nadekobotpm2start.sh && bash "$root/nadekobotpm2start.sh"
        echo ""
        sleep 2s
        bash "$root/linuxAIO.sh"
